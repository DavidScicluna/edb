import { Style } from '../../../common/types';
import { Theme } from '../../../theme/types';
import { DatePickerProps, Size } from './types';

type SizeStyle = { [key in Size]: Style };

type InvalidStyle = { invalid: Style };

// type DefaultStyle = { default: Style };

// type DateStyle = {
//   input: DefaultStyle & SizeStyle;
//   formLabel: DefaultStyle & SizeStyle;
//   // formHelperText: DefaultStyle & SizeStyle;
//   light: {
//     input: Style;
//     formLabel: Style;
//     // formHelperText: Style;
//   };
//   dark: {
//     input: Style;
//     formLabel: Style;
//     // formHelperText: Style;
//   };
// };

// type StyleDateProps = {
//   color: DateProps['color'];
//   // isDisabled: DateProps['isDisabled'];
//   // isFullWidth: DateProps['isFullWidth'];
// };

// export default (
//   theme: Theme,
//   { color = 'gray', isDisabled = false, isFullWidth = false }: StyleDateProps
// ): DateStyle => ({
//   input: {
//     default: {
//       'cursor': isDisabled ? 'not-allowed' : 'text',

//       'width': isFullWidth ? '100%' : 'auto',
//       'height': 'auto',

//       'minWidth': 'auto',
//       'minHeight': 'auto',
//       'maxWidth': 'none',
//       'maxHeight': 'none',

//       'opacity': isDisabled ? 0.5 : 1,

//       'borderStyle': 'solid',
//       'borderWidth': '2px',

//       // 'outline': 'none',
//       // 'outlineWidth': '0px',
//       // 'outlineStyle': 'dashed',

//       'fontWeight': 'normal',
//       'textTransform': 'normal',
//       'whiteSpace': 'nowrap',
//       'lineHeight': 'normal',

//       'transition': `${theme.transition.duration.faster} ${theme.transition.easing['ease-out']} !important`,

//       '&:focus': {
//         boxShadow: 'none'
//       },

//       '& svg': {
//         transition: `${theme.transition.duration.faster} ${theme.transition.easing['ease-out']} !important`
//       }
//     },
//     sm: {
//       fontSize: 'sm',

//       borderRadius: 'sm',

//       padding: theme.space[1]

//       // '&:focus': {
//       //   outlineOffset: '4px'
//       // }
//     },
//     md: {
//       fontSize: 'md',

//       borderRadius: 'base',

//       padding: `${theme.space[1.5]} ${theme.space[2]}`

//       // '&:focus': {
//       //   outlineOffset: '5px'
//       // }
//     },
//     lg: {
//       fontSize: 'lg',

//       borderRadius: 'lg',

//       padding: theme.space[2]

//       // '&:focus': {
//       //   outlineOffset: '6px'
//       // }
//     }
//   },
//   formLabel: {
//     default: {
//       opacity: isDisabled ? 0.5 : 1,

//       fontWeight: 'medium',
//       textTransform: 'uppercase',
//       whiteSpace: 'nowrap',
//       lineHeight: 'normal',

//       margin: 0,

//       transition: `${theme.transition.duration.faster} ${theme.transition.easing['ease-out']} !important`
//     },
//     sm: {
//       fontSize: 'xs',

//       pb: theme.space[0.5]
//     },
//     md: {
//       fontSize: 'sm',

//       pb: theme.space[0.75]
//     },
//     lg: {
//       fontSize: 'md',

//       pb: theme.space[1]
//     }
//   },
//   // formHelperText: {
//   //   default: {
//   //     opacity: isDisabled ? 0.5 : 1,

//   //     fontWeight: 'medium',
//   //     textTransform: 'uppercase',
//   //     whiteSpace: 'nowrap',
//   //     lineHeight: 'normal',

//   //     transition: `${theme.transition.duration.faster} ${theme.transition.easing['ease-out']} !important`
//   //   },
//   //   sm: {
//   //     fontSize: 'xs',

//   //     pt: theme.space[0.5]
//   //   },
//   //   md: {
//   //     fontSize: 'sm',

//   //     pt: theme.space[0.75]
//   //   },
//   //   lg: {
//   //     fontSize: 'md',

//   //     pt: theme.space[1]
//   //   }
//   // },
//   light: {
//     input: {
//       default: {
//         'borderColor': 'gray.200',
//         'backgroundColor': 'gray.100',
//         'color': 'gray.400',

//         '&:hover': {
//           borderColor: isDisabled ? 'gray.200' : `${color}.400`,
//           backgroundColor: isDisabled ? 'gray.100' : `${color}.100`,
//           color: isDisabled ? 'gray.400' : `${color}.400`
//         },

//         '&:focus': {
//           borderColor: isDisabled ? 'gray.200' : `${color}.400`,
//           backgroundColor: isDisabled ? 'gray.100' : `${color}.100`,
//           color: isDisabled ? 'gray.400' : `${color}.400`
//         }
//       },
//       invalid: {
//         'borderColor': isDisabled ? 'gray.200' : 'red.400',
//         'backgroundColor': isDisabled ? 'gray.100' : 'red.100',
//         'color': isDisabled ? 'gray.400' : 'red.400',

//         '&:hover': {
//           borderColor: isDisabled ? 'gray.200' : 'red.400',
//           backgroundColor: isDisabled ? 'gray.100' : 'red.100',
//           color: isDisabled ? 'gray.400' : 'red.400'
//         },

//         '&:focus': {
//           borderColor: isDisabled ? 'gray.200' : 'red.400',
//           backgroundColor: isDisabled ? 'gray.100' : 'red.100',
//           color: isDisabled ? 'gray.400' : 'red.400'
//         }
//       }
//     },
//     formLabel: {
//       default: {
//         color: `gray.${isDisabled ? 400 : 900}`
//       },
//       invalid: {
//         color: 'red.400'
//       }
//     }
//     // formHelperText: {
//     //   color: 'red.400'
//     // }
//   },
//   dark: {
//     input: {
//       default: {
//         'borderColor': 'gray.700',
//         'backgroundColor': 'gray.800',
//         'color': 'gray.400',

//         '&:hover': {
//           borderColor: isDisabled ? 'gray.700' : `${color}.400`,
//           backgroundColor: isDisabled ? 'gray.800' : `${color}.100`,
//           color: isDisabled ? 'gray.400' : `${color}.400`
//         },

//         '&:focus': {
//           borderColor: isDisabled ? 'gray.700' : `${color}.400`,
//           backgroundColor: isDisabled ? 'gray.800' : `${color}.100`,
//           color: isDisabled ? 'gray.400' : `${color}.400`
//         }
//       },
//       invalid: {
//         'borderColor': isDisabled ? 'gray.700' : 'red.400',
//         'backgroundColor': isDisabled ? 'gray.800' : 'red.100',
//         'color': isDisabled ? 'gray.400' : 'red.400',

//         '&:hover': {
//           borderColor: isDisabled ? 'gray.700' : 'red.400',
//           backgroundColor: isDisabled ? 'gray.800' : 'red.100',
//           color: isDisabled ? 'gray.400' : 'red.400'
//         },

//         '&:focus': {
//           borderColor: isDisabled ? 'gray.700' : 'red.400',
//           backgroundColor: isDisabled ? 'gray.800' : 'red.100',
//           color: isDisabled ? 'gray.400' : 'red.400'
//         }
//       }
//     },
//     formLabel: {
//       default: {
//         color: `gray.${isDisabled ? 500 : 50}`
//       },
//       invalid: {
//         color: 'red.500'
//       }
//     }
//     // formHelperText: {
//     //   color: 'red.500'
//     // }
//   }
// });
