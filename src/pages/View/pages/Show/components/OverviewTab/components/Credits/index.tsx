...
const Credits = (props: CreditsProps): ReactElement => {
  ...
  const renderCredits: ListItem[] = _.compact([
    (!_.isNil(createdBy) && !_.isEmpty(createdBy)) || isLoading
      ? {
          label: 'Created By',
          children: <CreatedBy key={`tv-show-${show?.id}-created-by`} people={createdBy} isLoading={isLoading} />
        }
      : undefined,
    ...
    (!_.isNil(executiveProducers) && !_.isEmpty(executiveProducers)) || isLoading
      ? {
          ...
          children: (
            <Credit key={`tv-show-${show?.id}-executive-producers`} people={executiveProducers} isLoading={isLoading} />
          )
        }
      : undefined,
