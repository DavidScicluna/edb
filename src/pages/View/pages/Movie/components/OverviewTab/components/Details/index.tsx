...
const Details = ({ movie, isLoading = true }: DetailsProps): ReactElement => {
  ...
  const renderDetails: ListItem[] = _.compact([
    ...
    (!_.isNil(movie?.revenue) && !_.isEmpty(movie?.revenue)) || isLoading
      ? {
          label: 'Revenue',
          children: <Revenue key={`movie-${movie?.id}-revenue`} revenue={movie?.revenue} isLoading={isLoading} />
        }
      : undefined,
    (!_.isNil(movie?.original_language) && !_.isEmpty(movie?.original_language)) || isLoading
      ? {
          ...
          children: (
            <Language key={`movie-${movie?.id}-language`} language={movie?.original_language} isLoading={isLoading} />
          )
        }
      : undefined,
    (!_.isNil(spokenLanguages) && !_.isEmpty(spokenLanguages)) || isLoading
      ? {
          ...
          children: <Languages key={`movie-${movie?.id}-languages`} languages={spokenLanguages} isLoading={isLoading} />
        }
      : undefined
  ]);
