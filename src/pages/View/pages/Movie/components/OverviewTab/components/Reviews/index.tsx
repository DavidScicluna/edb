...
const Reviews = ({ movie, reviews = [], isLoading = true, onChangeTab }: ReviewsProps): ReactElement => {
  ...
  return (
    <Panel isFullWidth>
      {{
        header: {
          title: reviews.length > 0 ? 'Latest Review' : movieUserReviews.length > 0 ? 'My Latest Review' : 'Reviews',
          actions:
          ...
          !isLoading && !_.isNil(otherReviews[0]) && !_.isEmpty(otherReviews[0]) ? (
            ...
            <Empty
              ...
              label={
                isSm
                  ? 'Write a review'
                  : `You currently have not written any reviews ${movie?.title ? `for "${movie.title}"` : ''}`
              }
              description={
                isSm
