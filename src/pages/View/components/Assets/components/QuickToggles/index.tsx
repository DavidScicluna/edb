...
const QuickToggles = (props: QuickTogglesProps): ReactElement => {
  ...
  const { assets, openedPanels, isLoading = true, onTogglePanel, onToggleAllPanels } = props;
    ...
    >
      ...
      <HStack width={`calc(100% - ${buttonWidth + 34}px)`} justifyContent='stretch'>
        ...
        <Center width={`calc(100% - ${textWidth + 8}px)`} height='100%'>
          ...
          >
            {[...(!isLoading ? assets : _.range(0, 5))].map((asset, index) => (
              <Asset
                key={index}
                asset={typeof asset !== 'number' ? asset : undefined}
                isLoading={isLoading}
                isDisabled={typeof asset === 'number'}
      ...
      <Button ref={buttonRef} isDisabled={isLoading} onClick={() => onToggleAllPanels()} size='sm' variant='text'>
        {assets.length === openedPanels ? 'Hide all' : 'Show all'}
      </Button>
    </HStack>
  );
};
