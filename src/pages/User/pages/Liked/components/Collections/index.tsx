  ...
  return (
    <VStack width='100%' spacing={4}>
      {collections.length === 0 ? (
          ...
          {({ displayMode }) =>
            collections
              ...
              .map((collection: Collection) =>
                displayMode === 'list' ? (
                  <HorizontalCompanyPoster key={collection.id} collection={collection} isLoading={false} />
                ) : (
                  <VerticalCompanyPoster key={collection.id} collection={collection} isLoading={false} />
                )
              )
          }
        </VerticalGrid>
        ...
        <VerticalGrid>
          {({ displayMode }) =>
            _.range(0, collections && collections.length > 0 ? collections.length : 20).map((_dummy, index: number) =>
              displayMode === 'list' ? (
