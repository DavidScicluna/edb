...
const RecentlyViewedMovies: FC = () => {
	...
	return recentlyViewedMovies.length === 0 ? (
		<QueryEmpty
			color={color}
			colorMode={colorMode}
			borderWidth='2px'
		...
		<VStack width='100%' spacing={spacing}>
			...
			<Center width={isSm ? '100%' : 'auto'}>
				<LoadMore
					...
					total={recentlyViewedMovies.length}
					label={formatMediaTypeLabel({ type: 'multiple', mediaType: 'movie' })}
					isLoading={false}
					isButtonVisible={visibleDebounced <= recentlyViewedMovies.length}
